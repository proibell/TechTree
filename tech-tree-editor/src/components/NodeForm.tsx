      ...
      <div className="space-y-3">
              ...
              {editingPort?.portId === port.id ? (
                <>
                  <input
                    ...
                    className={`w-24 px-2 py-1 text-sm border ${pixelStyle ? '' : 'rounded'} ${pixelStyle ? (darkMode ? 'bg-gray-700 border-gray-500 text-gray-200' : 'bg-gray-600 border-gray-400 text-gray-200') : (darkMode ? 'bg-gray-600 border-gray-500 text-gray-100' : 'border-gray-300')}`}
                    style={{ textTransform: pixelStyle ? 'uppercase' : 'none' }}
                  />
                  <select
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="连接点名称"
            className={`w-24 px-2 py-1 text-sm border ${pixelStyle ? '' : 'rounded'} ${pixelStyle ? (darkMode ? 'bg-gray-700 border-gray-500 text-gray-200' : 'bg-gray-600 border-gray-400 text-gray-200') : (darkMode ? 'bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-400' : 'border-gray-300')} ${selectedNode.completed ? 'opacity-50 cursor-not-allowed' : ''}`}
